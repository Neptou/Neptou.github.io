...
import type { Hotel } from "@/components/HotelsTable";
...
export default function HotelsPage() {
  ...
  async function handleSearch(e: React.FormEvent) {
    ...
    try {
      const res = await authFetch(`/admin/hotels?${params}`);
      ...
      <main className="max-w-7xl mx-auto px-6 py-8">
        <div className="mb-6">
          ...
          <p className="text-gray-400 text-sm mt-1">
            Manage accommodation listings shown in the app
          </p>
        </div>
        ...
        {hotels === null && !loading && (
          <p className="text-gray-500 text-center py-16">
            Use the filters above and click Search to load hotels.
          </p>
        )}
        ...
        {hotels !== null && (
          <>
            <p className="text-gray-400 text-sm mb-4">
              ...
              {hotels.length === 200 && " — showing first 200, narrow your filters for more"}
            </p>
            <HotelsTable
              hotels={hotels}
              onHotelsChange={(updater) => setHotels((prev) => updater(prev ?? []))}
            />
          </>
        )}
      </main>
